...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates locales bzip2 \
...
RUN chmod +x /usr/local/bin/docker-entrypoint.sh /usr/local/bin/start-stack.sh /usr/local/bin/vvz-configure-pg-hba.sh \
    ...
    groupmod -g 60080 grp1cv8; \
    usermod -u 60080 -g grp1cv8 usr1cv8; \
    getent group pcscd >/dev/null && usermod -aG pcscd usr1cv8 || true; \
    if id -u postgres >/dev/null 2>&1; then \
      groupmod -g 60081 postgres; \
      usermod -u 60081 -g postgres postgres; \
    fi
