...
mkdir -p "$STAGE/usr/libexec/vvz-1csrv-postgres" "$STAGE/lib/systemd/system" "$STAGE/etc/cron.d"
...
cp -f "$ROOT/scripts/pgsql1c-backup.sh" "$STAGE/usr/libexec/vvz-1csrv-postgres/pgsql1c-backup"
cp -f "$ROOT/scripts/pgsql1c-host-users.sh" "$STAGE/usr/libexec/vvz-1csrv-postgres/pgsql1c-host-users.sh"
cp -f "$ROOT/scripts/pgsql1c-verify-storage.sh" "$STAGE/usr/libexec/vvz-1csrv-postgres/pgsql1c-verify-storage"
cp -f "$ROOT/docker/srv1cv83.default" "$STAGE/usr/share/vvz-1csrv-postgres/srv1cv83.default"
chmod +x "$STAGE/usr/libexec/vvz-1csrv-postgres/stack-start" "$STAGE/usr/libexec/vvz-1csrv-postgres/stack-stop" \
  "$STAGE/usr/libexec/vvz-1csrv-postgres/stack-status" "$STAGE/usr/libexec/vvz-1csrv-postgres/pgsql1c-backup" \
  "$STAGE/usr/libexec/vvz-1csrv-postgres/pgsql1c-verify-storage"
