...
Скрипт **`docker/start-stack.sh`** после старта PostgreSQL крутит **супервизор ragent**: при завершении процесса пишет в лог контейнера краткую диагностику (`status`, `ps`, хвост `postgres.log`, имена файлов под `1cv8`) и перезапускает 1С, не убивая контейнер. В **`docker-compose.yml`** для сервиса **`app`** задано **`restart: unless-stopped`**.

**`SRV1CV8_DEBUG`**: в **`docker/srv1cv83.default`** по умолчанию **`0`**. Значение **`1`** добавляет **`ragent -debug`** (серверная отладка из конфигуратора). Если после включения отладки ragent стабильно падает, проверьте лицензию, порты и логи в **`PGSQL1C_VAR/1cv8`**, затем пересоберите образ.
